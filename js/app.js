...
promptExportPricing._proceed=function(){
  ...
  ov.innerHTML=`
    <div style="background:#fff;border-radius:12px;padding:28px;width:340px;box-shadow:0 20px 60px rgba(0,0,0,0.3);">
      <div style="font-family:'Barlow Condensed',sans-serif;font-size:19px;font-weight:700;color:#0A3D8F;margin-bottom:4px;">Export Bill of Materials</div>
      <div style="font-size:12.5px;color:#64748b;margin-bottom:18px;">Include MAP pricing in this PDF?</div>
      <button id="expWithPrice" style="width:100%;padding:11px;margin-bottom:8px;border:none;border-radius:8px;background:#0A3D8F;color:#fff;font-weight:600;font-size:13px;cursor:pointer;">With Pricing</button>
      <button id="expNoPrice" style="width:100%;padding:11px;margin-bottom:8px;border:1.5px solid #cbd5e1;border-radius:8px;background:#fff;color:#1e293b;font-weight:600;font-size:13px;cursor:pointer;">Without Pricing <span style="font-weight:400;color:#94a3b8;">(for distribution)</span></button>
      <button id="expCancel" style="width:100%;padding:9px;border:none;background:transparent;color:#94a3b8;font-size:12px;cursor:pointer;">Cancel</button>
    </div>`;
  document.body.appendChild(ov);
  const close=()=>ov.remove();
  document.getElementById('expWithPrice').onclick=()=>{close();exportReport(true);};
  document.getElementById('expNoPrice').onclick=()=>{close();exportReport(false);};
  document.getElementById('expCancel').onclick=close;
  ov.addEventListener('click',e=>{if(e.target===ov)close();});
}

function exportReport(includePricing=true){
  const counts={};
  placements.forEach(pl=>{if(!counts[pl.product.sku])counts[pl.product.sku]={product:pl.product,qty:0};counts[pl.product.sku].qty++;});
  mapMarkers.forEach(m=>{if(!counts[m.product.sku])counts[m.product.sku]={product:m.product,qty:0};counts[m.product.sku].qty++;});
  const items=Object.values(counts);
  ...
  loadScript('https://cdnjs.cloudflare.com/ajax/libs/jspdf/2.5.1/jspdf.umd.min.js',()=>{
    loadScript('https://cdnjs.cloudflare.com/ajax/libs/html2canvas/1.4.1/html2canvas.min.js',()=>{
      setTimeout(()=>buildPDF(items,prog,setP,includePricing).catch(err=>{
...
async function buildPDF(items,prog,setP,includePricing=true){
  ...
  function drawFooter(pageNum){
    ...
    pdf.text(includePricing?'MAP pricing for distribution reference only':'Quantities for distribution reference only',M,PH-3.5);
  }
  ...
  pdf.text(includePricing?'BILL OF MATERIALS':'MATERIALS LIST',M+40,13);
  setTxt([80,95,115]);pdf.setFont('helvetica','normal');pdf.setFontSize(7.5);
  pdf.text((includePricing?'Complete product list with MAP pricing — ':'Complete product list — ')+proj,M+40,20);
  ...
  items.forEach((it,i)=>{
    ...
    const descLimit=includePricing?70:98;
    const desc2=it.product.description.length>descLimit?it.product.description.substring(0,descLimit-1)+'…':it.product.description;
  ...
  // ── Total row (pricing only — a materials-only list has no dollar total to show) ────────
  if(includePricing){
